...
<LessonTemplate {lesson} {onBack}>
	<div class="flex min-h-[400px] flex-col gap-8 p-4">
		<div class="mb-4 text-center">
			<p class="text-lg text-slate-700">
				{config.instructions || 'Σύρετε τα αντικείμενα στη σωστή θέση.'}
			</p>
		</div>
			...
			{#each pieces as piece}
				{#if !piece.dropped}
					<div
						...
						class="h-20 w-20 {piece.color} flex cursor-grab items-center justify-center font-bold text-white shadow-md transition-transform select-none hover:scale-105"
		...
		<div class="mt-auto flex flex-wrap justify-center gap-8 border-t border-slate-200 pt-12">
			{#each zones as zone}
				<div
					...
					class="relative flex h-32 w-32 flex-col items-center justify-center border-4 border-dashed transition-colors
				...
				>
					...
					{:else}
						<span class="font-bold text-slate-300">{zone.name}</span>
					{/if}
