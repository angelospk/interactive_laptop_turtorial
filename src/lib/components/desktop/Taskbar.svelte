...
>
	...
	<div class="flex items-center gap-2">
		...
		{#each apps as app}
			...
			<div class="group relative">
				...
				{#if isOpen}
					<div
						class="absolute bottom-0 left-1/2 h-1 w-4 -translate-x-1/2 rounded-full bg-blue-400"
					></div>
				{/if}
			</div>
		{/each}
	...
	<div class="flex h-full items-center gap-2">
		<button
			class="flex h-full items-center gap-2 rounded px-2 py-1 transition-colors hover:bg-slate-700 {showQuickSettings
				? 'bg-slate-700'
				: ''}"
