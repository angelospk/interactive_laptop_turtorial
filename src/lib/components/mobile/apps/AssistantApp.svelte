...
<div data-testid="assistant-app" class="flex h-full flex-col bg-slate-50">
	...
	<div class="flex flex-1 flex-col gap-3 overflow-y-auto p-4">
		<!-- Assistant bubble -->
		<p class="max-w-[85%] self-start rounded-2xl rounded-tl-sm bg-white px-4 py-3 text-base text-slate-800 shadow-sm">
			{greeting}
		</p>

		{#if picked}
			<p class="max-w-[85%] self-end rounded-2xl rounded-tr-sm bg-indigo-600 px-4 py-3 text-base text-white shadow-sm">
				{picked.text}
			</p>
			<p
	...
	<div class="shrink-0 space-y-2 border-t border-slate-200 bg-white p-3">
		...
		{#each phrases as phrase (phrase.id)}
			<button
				...
				class={cn(
					'flex w-full min-h-[52px] items-center justify-center rounded-full border px-4 py-2 text-center text-base font-medium transition focus-visible:ring-4 focus-visible:ring-blue-400 focus-visible:outline-none',
					picked?.id === phrase.id
