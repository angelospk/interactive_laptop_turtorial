<script lang="ts">
	...
	let completedTasks = $state<boolean[]>([false, false, false, false]);

	const tasks = [
		{
			title: 'Δημιουργία Φακέλου',
			description: 'Φτιάξε έναν νέο φάκελο με όνομα "Εργασία" στον κεντρικό χώρο.',
			check: () =>
				items.some((i) => i.name === 'Εργασία' && i.type === 'folder' && i.parentId === 'root')
		},
		{
			title: 'Οργάνωση Αρχείων',
			description:
				'Μετάφερε (Αποκοπή & Επικόλληση) το αρχείο "Λίστα Ψώνια.txt" μέσα στον φάκελο "Έγγραφα".',
			check: () => items.some((i) => i.name === 'Λίστα Ψώνια.txt' && i.parentId === '1')
		},
		{
			title: 'Μετονομασία',
			description: 'Μετονόμασε τον φάκελο "Εικόνες" σε "Φωτογραφίες".',
			check: () => items.some((i) => i.name === 'Φωτογραφίες' && i.type === 'folder')
		},
		{
			title: 'Διαγραφή',
			description: 'Διέγραψε το αρχείο "Διακοπές.jpg".',
			check: () => !items.some((i) => i.name === 'Διακοπές.jpg')
		}
	];
	...
	function getPath(folderId: string): { id: string; name: string }[] {
		...
		// Simple one-level up check for now, recursive for full path
	...
	function deleteItem() {
		...
		if (item) {
			// If folder, delete contents too (simple version)
		...
		if (currentTaskIndex < tasks.length) {
			if (tasks[currentTaskIndex].check()) {
				if (!completedTasks[currentTaskIndex]) {
					...
					toast.success(`Μπράβο! Ολοκλήρωσες την άσκηση: ${tasks[currentTaskIndex].title}`);
					gameState.updateProgress('module4', ((currentTaskIndex + 1) / tasks.length) * 100);

					if (currentTaskIndex < tasks.length - 1) {
						setTimeout(() => {
							currentTaskIndex++;
							toast.info(`Επόμενη άσκηση: ${tasks[currentTaskIndex].title}`);
						}, 1500);
					} else {
						toast.success('Συγχαρητήρια! Έγινες ειδικός στην Αρχειοθέτηση!');
					}
				}
			}
		}
...
<div class="space-y-6">
	<Card>
		<CardHeader>
			<CardTitle>4. Ο Αρχειοθέτης (File Explorer)</CardTitle>
			<CardDescription>Οργάνωσε τα αρχεία σου όπως σε έναν πραγματικό υπολογιστή.</CardDescription>
		</CardHeader>
		<CardContent>
			<!-- Task Info -->
			<div class="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4">
				<h3 class="mb-2 font-bold text-blue-900">
					Άσκηση {currentTaskIndex + 1}: {tasks[currentTaskIndex].title}
				</h3>
				<p class="text-slate-700">{tasks[currentTaskIndex].description}</p>
				<div class="mt-2 h-2 w-full overflow-hidden rounded-full bg-blue-200">
					<div
						class="h-full bg-blue-600 transition-all duration-500"
						style="width: {(completedTasks.filter(Boolean).length / tasks.length) * 100}%"
					></div>
				</div>
			</div>
