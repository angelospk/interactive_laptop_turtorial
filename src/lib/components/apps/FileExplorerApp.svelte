<script lang="ts">
	...
	let items = $state<FileSystemItem[]>(initialFiles.length > 0 ? initialFiles : defaultItems);
	...
	function deleteItem() {
		...
		if (item) {
			...
			onAction('delete', { name: item.name });
			selectedItemId = null;
		}
	}
	...
	function copyItem(cut = false) {
		...
		onAction(cut ? 'cut' : 'copy', { id: selectedItemId });
	}
	...
	function handleDrop(e: DragEvent, targetFolderId: string) {
		...
		if (draggedId && draggedId !== targetFolderId) {
			...
			if (item) {
				...
				onAction('drag-drop', { name: item.name, targetFolderId });
			}
		}
	}
</script>
