...
_Τελευταία ενημέρωση: 2026-07-22. Βασίζεται σε χαρτογράφηση του codebase + έρευνα με Grok (live web search, βλ. §Έρευνα)._
...
> **Auto-detect ως βοήθημα, ποτέ σιωπηλά.** Το βέλτιστο pattern: _«Φαίνεται ότι χρησιμοποιείς κινητό Android — σωστά;»_ με μεγάλα κουμπιά [Ναι] / [Όχι, δείξε μου επιλογές], και δυνατότητα αλλαγής αργότερα από εμφανές σημείο.

Λόγοι: (α) η ανίχνευση κάνει λάθη (iPad, shared devices, ο παππούς μπαίνει από το laptop της κόρης για να μάθει για το κινητό _του_), (β) οι σιωπηλές αλλαγές μπερδεύουν και τρομάζουν χρήστες χαμηλής εξοικείωσης, (γ) WCAG: user control, όχι απροσδόκητες αλλαγές context. Σημαντικό σενάριο για εμάς: **η συσκευή που σερφάρει ≠ η συσκευή που θέλει να μάθει** — άρα η προτίμηση είναι ρύθμιση χρήστη, όχι ιδιότητα του browser.
...
Στόχος: η πλατφόρμα ξέρει (α) τι συσκευή πιθανόν έχει ο χρήστης, (β) τι συσκευή _θέλει να μάθει_, και τα μαθήματα φέρουν device tags.
...
2. **Detection utility** (`src/lib/utils/deviceDetect.ts`): `userAgentData` → UA fallback → touch heuristics (για iPad-ως-Mac). Επιστρέφει _υπόθεση_, όχι απόφαση.
...
5. **Filtering/badging στο UI**: στην αρχική, ενότητα «Για τη συσκευή σου»· στα modules, φιλτράρισμα ή badges (🖥️/📱) ανά μάθημα. Όχι απόκρυψη — _προτεραιοποίηση_ (ο χρήστης βλέπει πρώτα τα δικά του, αλλά όλα προσβάσιμα).
...
>
> **Πρόοδος 2026-07-22**: ολοκληρώθηκε το **Mac track** (ibm/532) — νέος lessonType `mac-simulation` (MacDesktop/MacWindow με «φανάρια», Dock με ένδειξη «τρέχει», γραμμή μενού, Spotlight, Finder, Ρυθμίσεις), state model `running`/`windowState`/`activeAppId` με κεντρικό δίδαγμα **κλείσιμο ≠ τερματισμός** (⌘Q / μενού). Module `mac` με 8 μαθήματα σε 3 ενότητες, κατηγορία «Mac υπολογιστής», truthful `mac` tags. Επίσης διορθώθηκε το i18n (dyc): συμπληρώθηκαν 181 keys στο `en.json` + parity contract el↔en.
...
4. **Mac track** — ✅ ΟΛΟΚΛΗΡΩΘΗΚΕ (2026-07-22): πλήρες `mac-simulation` track με 9 μαθήματα (Dock/Finder/Spotlight/traffic lights, κλείσιμο≠τερματισμός). Δες §Φάση 2 πρόοδο.
